...
parser = argparse.ArgumentParser(description='Test-image-deblurring')
...
parser.add_argument('--gpu', type=int, default=0, required=False, help='gpu index')
parser.add_argument('--n_threads', type=int, default=8, help='number of threads for data loading')
#'store_false'--代表save 默认值为ture， 保存结果
parser.add_argument('--save', action='store_false', help='Save deblurred image')
...
def test(args):
    ...
    my_model = networks.ResnetGenerator(3,3,ngf=params['n_feats'], n_blocks=params['n_resblocks'],
                                    adaNorm=params['adaNorm'],deformable=params['deformable'],dcn_ksize=params['deformable_kersize'])
